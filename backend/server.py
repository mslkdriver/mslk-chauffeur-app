...
def trip_to_response(trip: dict) -> TripResponse:
    ...
    return TripResponse(
        ...
        driver_vehicle_model=trip.get("driver_vehicle_model"),
        driver_vehicle_color=trip.get("driver_vehicle_color"),
        driver_vehicle_plate=trip.get("driver_vehicle_plate"),
...
@api_router.post("/driver/trips/{trip_id}/approach", response_model=TripResponse)
async def driver_approach(trip_id: str, current_user: dict = Depends(get_current_user)):
    """Driver starts approaching - Chauffeur en approche"""
    if current_user["role"] != UserRole.DRIVER.value:
        raise HTTPException(status_code=403, detail="Drivers only")
    ...
    await db.trips.update_one(
        {"id": trip_id},
        {"$set": {
            "status": TripStatus.APPROACHING.value,
            "updated_at": now
        }}
    )
    ...
    # Notify client that driver is approaching
    ...
    client_notification = {
        ...
        "message": f"Votre chauffeur {driver['name']} est en approche !",
        "driver_name": driver["name"],
        "driver_phone": driver.get("phone", ""),
        "driver_vehicle": f"{driver.get('vehicle_model', '')} {driver.get('vehicle_color', '')}",
        "driver_plate": driver.get("vehicle_plate", ""),
        "created_at": now,
        "read": False
    }
    ...
    await send_email_notification(
        trip["client_email"],
        "🚗 MSLK VTC - Votre chauffeur est en approche !",
...
@api_router.post("/driver/trips/{trip_id}/start", response_model=TripResponse)
async def driver_start_trip(trip_id: str, current_user: dict = Depends(get_current_user)):
    """Driver starts the trip - Course démarre"""
    if current_user["role"] != UserRole.DRIVER.value:
        raise HTTPException(status_code=403, detail="Drivers only")
...
@api_router.post("/driver/trips/{trip_id}/complete", response_model=TripResponse)
async def driver_complete_trip(trip_id: str, current_user: dict = Depends(get_current_user)):
    """Driver completes the trip - Course terminée"""
    if current_user["role"] != UserRole.DRIVER.value:
        raise HTTPException(status_code=403, detail="Drivers only")
    ...
    # Create notification for driver to collect payment
    driver_notification = {
        "id": str(uuid.uuid4()),
    ...
    await db.notifications.insert_one(driver_notification)
    
    # Create notification for client to pay
    client_notification = {
        "id": str(uuid.uuid4()),
        "type": "payment_due",
        "trip_id": trip_id,
        "client_email": trip["client_email"],
        "message": f"Course terminée ! Montant à payer : {trip['price']:.2f}€",
        "amount": trip["price"],
        "created_at": now,
        "read": False
    }
    ...
    await send_email_notification(
        trip["client_email"],
        "MSLK VTC - Course terminée",
        f"Bonjour {trip['client_name']},\n\nVotre course est terminée.\n\nMontant à régler au chauffeur : {trip['price']:.2f}€\n\nDépart : {trip['pickup_address']}\nArrivée : {trip['dropoff_address']}\n\nMerci d'avoir choisi MSLK VTC !\n\nL'équipe MSLK VTC"
    )
...
async def update_trip_status(trip_id: str, status_update: TripStatusUpdate, current_user: dict = Depends(get_current_user)):
    """Update trip status (legacy endpoint)"""
    if current_user["role"] != UserRole.DRIVER.value:
        raise HTTPException(status_code=403, detail="Drivers only")
...
# Update driver total trips (modify or reset)
class TotalTripsUpdate(BaseModel):
    total_trips: int

@api_router.put("/admin/drivers/{driver_id}/total-trips", response_model=UserResponse)
async def update_driver_total_trips(driver_id: str, update: TotalTripsUpdate, current_user: dict = Depends(get_current_user)):
    """Update or reset total trips count for a driver"""
    if current_user["role"] != UserRole.ADMIN.value:
        raise HTTPException(status_code=403, detail="Admins only")
    
    result = await db.users.update_one(
        {"id": driver_id, "role": UserRole.DRIVER.value},
        {"$set": {"total_trips": update.total_trips}}
    )
