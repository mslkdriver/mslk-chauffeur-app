...
export default function ClientLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(`${API}/auth/client/login`, loginData);
      localStorage.setItem("mslk_client_token", response.data.token);
      localStorage.setItem("mslk_client_user", JSON.stringify(response.data.user));
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("mslk_client_user", JSON.stringify(response.data.user));
      navigate("/client/espace");
      toast.success("Inscription réussie ! Bienvenue chez MSLK VTC");
    } catch (error) {
      ...
      <div className="flex-1 flex items-center justify-center px-4 pb-8">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <h1 className="font-heading text-3xl font-bold text-white mb-2" data-testid="client-login-title">
              Espace <span className="text-[#D4AF37]">Client</span>
            </h1>
            <p className="text-[#A1A1A1]">Gérez vos réservations en toute simplicité</p>
          </div>
            ...
            <Tabs defaultValue="login" className="w-full">
              <TabsList className="grid w-full grid-cols-2 bg-black mb-6">
                ...
                <TabsTrigger 
                  value="register" 
                  className="data-[state=active]:bg-[#D4AF37] data-[state=active]:text-black"
                  data-testid="tab-client-register"
                >
                  Inscription
                </TabsTrigger>
              </TabsList>
                ...
                <form onSubmit={handleRegister} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      required
                      data-testid="client-register-confirm"
                    />
                  </div>
                  <Button 
                    type="submit" 
                    className="btn-gold w-full h-12" 
                    disabled={loading}
                    data-testid="btn-client-register"
                  >
                    {loading ? <div className="spinner w-5 h-5" /> : "Créer mon compte"}
                  </Button>
                </form>
              </TabsContent>
            </Tabs>
          ...
          <div className="mt-6 text-center">
            <p className="text-[#A1A1A1] text-xs mb-2">Avantages de votre espace client :</p>
            <div className="flex flex-wrap justify-center gap-2">
              <span className="text-[#D4AF37] text-xs bg-[#D4AF37]/10 px-2 py-1 rounded">Réservation rapide</span>
              <span className="text-[#D4AF37] text-xs bg-[#D4AF37]/10 px-2 py-1 rounded">Historique complet</span>
              <span className="text-[#D4AF37] text-xs bg-[#D4AF37]/10 px-2 py-1 rounded">Infos pré-remplies</span>
            </div>
          </div>
        </div>
      </div>
